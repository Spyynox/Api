<template>
	<div class="text-md-center">
        <div class="card">
            <div class="card-body">
                <p class="full-name">Je suis l'utilisateur n° {{ posts.id }}</p>
                <p class="full-name">{{ posts.name }}</p>
                <p class="username">@{{ posts.username }}</p>
            </div>
        </div>
	</div>	
</template>
...
<script>
    ...
    export default {
        ...
        created() {
            axios.get(`http://212.47.241.143:5000/user/me`, headers)
            .then(response => {
